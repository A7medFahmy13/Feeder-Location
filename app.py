...
if selected_zones:
    ...
    # ✅ فلتر الشركة (لو موجود عمود company أو linked_name في النقاط)
    company_col = None
    for c in ["company", "linked_name", "owner"]:
        if c in df_points_filtered.columns:
            company_col = c
            break

    if company_col:
        companies = df_points_filtered[company_col].dropna().unique()
        selected_companies = st.multiselect("🏢 اختر الشركة", companies)

        if selected_companies:
            df_points_filtered = df_points_filtered[df_points_filtered[company_col].isin(selected_companies)]

            # ✅ فلتر feeder_id بعد الشركة
            if "feeder_id" in df_points_filtered.columns:
                feeder_ids = df_points_filtered["feeder_id"].dropna().unique()
...
# ✅ تجميع النقاط
marker_cluster = MarkerCluster(name="تجميع النقاط").add_to(m)
...
# ✅ الاتجاهات إلى نقطة مختارة (لو فيه عمود description)
if "description" in df_points_filtered.columns and not df_points_filtered.empty:
    selected_point = st.selectbox("➡️ اختر نقطة لعرض الاتجاهات", df_points_filtered["description"].dropna().unique())

    if selected_point:
        selected_point_row = df_points_filtered[df_points_filtered["description"] == selected_point].iloc[0]
        folium.PolyLine(
            locations=[(18.2, 42.5), (selected_point_row["latitude"], selected_point_row["longitude"])],
            color="red",
            weight=4,
            opacity=0.7,
            tooltip="الاتجاه إلى النقطة"
        ).add_to(m)

# ✅ عناصر التحكم
HeatMap([[row.latitude, row.longitude] for row in df_points_filtered.itertuples()],
        radius=15, name="خريطة الحرارة").add_to(m)
